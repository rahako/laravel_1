...
​
    <div class="edit-page">
        <div class="form-balance edit-balance">
            <form action="{{route('homebudget.update',$homebudget->id)}}" method="POST">
              @csrf
              @method('put')
                ...
                @if($errors->has('date')) <span class="error">{{$error->first('date')}}</span>@endif

                <label for="category_id">カテゴリ:</label>
                <select name="category" id="category">
                   @foreach($categories as $category)
                    <option value='{{$category->id}}' {{$category->id  == $homebudget->category_id ? 'selectd' : ''}}>
                      {{$category->name}}
                    </option>
                    @endforeach
                </select>
